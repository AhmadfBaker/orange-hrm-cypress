...
class Table{
    element = {
        tableRecord:() => cy.get('.oxd-table-body'),
        nextBtn:()=>cy.get(':nth-child(3) > .oxd-pagination-page-item')
    }
    recourde= []
    storeRecourde(TableHeader)
    {
        this.element.tableRecord().children().each(($el,index)=>{
            let obj ={
                index:index, 
            }
            let i =1; 
        ...
        for(let k=0;k<res.length;k++)
        {
            ...
            for (const key1 in TableHeader) {
                if(this.recourde[k][key1] != "")
                    this.element.tableRecord().children().eq(k).contains(this.recourde[res[k].index][key1])
            }
        }
